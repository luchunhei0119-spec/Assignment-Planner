...
export async function POST(req: NextRequest) {
  let client;
  try {
    client = await getClient(req, { cache: true });
  } catch (e) {
    const msg = e instanceof Error ? e.message : '';
    if (msg.startsWith('SERVER_CONFIG')) return serverError(msg.replace('SERVER_CONFIG: ', ''));
    return unauthorized();
  }
...
Return only valid JSON, no explanation.`,
      ...
      const match = raw.match(/\{[\s\S]*\}/);
      if (!match) return serverError('Could not parse response');
      try {
        ...
        return serverError('Response was too long, try fewer key points');
      }
    }
